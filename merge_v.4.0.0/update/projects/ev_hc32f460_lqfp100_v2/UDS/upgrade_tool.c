...
*     -> (51 01 直接应答: 立即 DONE | 静默: WAIT_5101 等补发 51 01) -> DONE
*     -> DONE_DELAY(静默3s, 丢弃残留心跳) -> LISTEN
...
typedef enum {
    ...
    TOOL_ST_DONE_DELAY,     /* 升级成功后静默期: 等新 APP 心跳稳定再恢复版本判断 */
...
static void st_ecu_reset(void)
{
    ...
    if (txn == TOOL_TXN_OK) {
        /* UdsTester 已校验肯定响应 SID = 请求SID + 0x40, 故 11 01 的 TXN_OK
         * 即代表 51 01 复位 ACK 已到: 立即判成功, 不再进 WAIT_5101 白等 15s */
        UdsTester_ResetTxn();
        TOOL_I("51 01 received as 11 01 resp, upgrade done");
        status_report(TOOL_STEP_DONE, 100U, 0U);
        s_flow_retry = 0;
        s_in_boot = 0;
        goto_state(TOOL_ST_DONE_DELAY);
    } else if (txn == TOOL_TXN_ERR) {
        tool_txn_err_t err = UdsTester_TxnError();
        UdsTester_ResetTxn();
        if (err == TOOL_TXN_ERR_TIMEOUT) {
...
/* 升级成功后静默期: 丢弃升级前残留心跳, 等新 APP 心跳稳定后再恢复版本判断 */
static void st_done_delay(void)
{
    if (nbDelay_IsComplete(&s_done_tmr)) {
        ...
        s_hb_flag = 0U;         /* 丢弃残留/在途心跳, 只认静默期结束后收到的心跳 */
